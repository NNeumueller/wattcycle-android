package com.wattcycle.ble

import java.nio.ByteBuffer
import java.nio.ByteOrder

object WattcycleProtocol {
    // BLE UUIDs
    const val SERVICE_UUID = "0000fff0-0000-1000-8000-00805f9b34fb"
    const val WRITE_UUID = "0000fff2-0000-1000-8000-00805f9b34fb"
    const val NOTIFY_UUID = "0000fff1-0000-1000-8000-00805f9b34fb"
    const val AUTH_UUID = "0000fffa-0000-1000-8000-00805f9b34fb"
    
    // Protocol constants
    const val FRAME_HEAD: Byte = 0x7E
    const val FRAME_TAIL: Byte = 0x0D
    const val FUNC_READ: Byte = 0x03
    const val DEVICE_ADDR: Byte = 0x01
    
    // DP Addresses
    const val DP_ANALOG_QUANTITY = 140  // 0x8C
    const val DP_PRODUCT_INFO = 146     // 0x92
    
    val AUTH_KEY = "HiLink".toByteArray()
    val DEVICE_NAME_PREFIXES = listOf("XDZN", "WT")
    
    // Full Modbus CRC16 lookup tables
    private val CRC_HI = intArrayOf(
        
    )
    
    private val CRC_LO = intArrayOf(
        
    )
    
    fun modbusCrc16(data: ByteArray): Int {
        var crcHi = 0xFF
        var crcLo = 0xFF
        for (b in data) {
            val idx = (crcHi xor (b.toInt() and 0xFF)) and 0xFF
            crcHi = (crcLo xor CRC_HI[idx]) and 0xFF
            crcLo = CRC_LO[idx] and 0xFF
        }
        return ((crcLo shl 8) or crcHi) and 0xFFFF
    }
    
    fun buildReadFrame(address: Int): ByteArray {
        val buf = ByteBuffer.allocate(11).order(ByteOrder.BIG_ENDIAN)
        buf.put(FRAME_HEAD)
        buf.put(0x00)  // version
        buf.put(DEVICE_ADDR)
        buf.put(FUNC_READ)
        buf.putShort(address.toShort())
        buf.putShort(0)  // read_count
        
        val payload = buf.array().copyOf(8)
        val crc = modbusCrc16(payload)
        buf.putShort(crc.toShort())
        buf.put(FRAME_TAIL)
        
        return buf.array()
    }
    
    fun parseFrame(data: ByteArray): WattFrame? {
        if (data.size < 11) return null
        if (data[0] != FRAME_HEAD || data[data.size - 1] != FRAME_TAIL) return null
        
        val buf = ByteBuffer.wrap(data).order(ByteOrder.BIG_ENDIAN)
        buf.position(4)
        val startAddr = buf.short.toInt() and 0xFFFF
        val dataLen = buf.short.toInt() and 0xFFFF
        
        val frameData = ByteArray(dataLen)
        buf.get(frameData)
        
        return WattFrame(
            version = data[1].toInt() and 0xFF,
            address = data[2].toInt() and 0xFF,
            functionCode = data[3].toInt() and 0xFF,
            startAddress = startAddr,
            dataLength = dataLen,
            data = frameData
        )
    }
    
    fun parseAnalogQuantity(data: ByteArray): AnalogQuantityData? {
        try {
            val buf = ByteBuffer.wrap(data).order(ByteOrder.BIG_ENDIAN)
            
            val cellCount = buf.get().toInt() and 0xFF
            val cellVoltages = mutableListOf<Float>()
            repeat(cellCount) {
                cellVoltages.add((buf.short.toInt() and 0xFFFF) / 1000.0f)
            }
            
            val tempCount = buf.get().toInt() and 0xFF
            val mosTemp = ((buf.short.toInt() and 0xFFFF) - 2730) / 10.0f
            val pcbTemp = ((buf.short.toInt() and 0xFFFF) - 2730) / 10.0f
            
            // Skip cell temperatures
            repeat(tempCount - 2) {
                buf.short
            }
            
            // Parse current
            val currentBytes = ByteArray(2)
            buf.get(currentBytes)
            val current = parseCurrentNegative(currentBytes, 0).first
            
            val voltage = (buf.short.toInt() and 0xFFFF) / 100.0f
            val remaining = (buf.short.toInt() and 0xFFFF) / 10.0f
            val total = (buf.short.toInt() and 0xFFFF) / 10.0f
            buf.short // cycle number
            buf.short // design capacity
            val soc = buf.short.toInt() and 0xFFFF
            
            return AnalogQuantityData(
                soc = soc,
                voltage = voltage,
                current = current,
                remainingCapacity = remaining,
                totalCapacity = total,
                cellVoltages = cellVoltages,
                mosTemperature = mosTemp,
                pcbTemperature = pcbTemp
            )
        } catch (e: Exception) {
            return null
        }
    }
    
    private fun parseCurrentNegative(data: ByteArray, offset: Int): Pair<Float, Int> {
        val b0 = data[offset].toInt() and 0xFF
        val b1 = data[offset + 1].toInt() and 0xFF
        val isNegative = (b0 and 0x80) != 0
        val hasDecimal = (b0 and 0x40) != 0
        val raw = b1 or ((b0 and 0x3F) shl 8)
        var current = if (hasDecimal) raw / 10.0f else raw.toFloat()
        if (isNegative) current = -current
        return Pair(current, offset + 2)
    }
}

data class WattFrame(
    val version: Int,
    val address: Int,
    val functionCode: Int,
    val startAddress: Int,
    val dataLength: Int,
    val data: ByteArray
)

data class AnalogQuantityData(
    val soc: Int,
    val voltage: Float,
    val current: Float,
    val remainingCapacity: Float,
    val totalCapacity: Float,
    val cellVoltages: List<Float>,
    val mosTemperature: Float,
    val pcbTemperature: Float
)

data class BatteryData(
    val name: String,
    val address: String,
    val soc: Int = 0,
    val voltage: Float = 0f,
    val current: Float = 0f,
    val remainingCapacity: Float = 0f,
    val totalCapacity: Float = 0f,
    val cellVoltages: List<Float> = emptyList(),
    val temperature: Float = 0f,
    val isConnected: Boolean = false
)
